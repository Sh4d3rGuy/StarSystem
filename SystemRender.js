...
StarS.SystemRender.prototype = {
   ...
   buildRenderObjects: function (  ) {
      ...
      //var orbits = new THREE.LineSegments(
      //geomOrbits,
      //   this.getOrbitMaterial()
      //);
      ...
      for (var i = 0; i < this.system.planetList.length; i++) {
         ...
         //var translate = this.system.planetList[i].getPosition(100);
         //planetGeom.translate(translate.x, translate.y, translate.z);
   ...
   createStar: function ( star ) {
      ...
      //var geom = new THREE.PlaneBufferGeometry( width, height, 1, 1 );
      var geom = this.createStarGeometry( star, 0 );

      var mat = this.getStarMaterial();//new THREE.MeshBasicMaterial({ color: new THREE.Color('#ffff00') });

      var starObject = new THREE.Object3D();
      var stars = new THREE.Points( geom, mat );
      //starObject.add(new THREE.Mesh(geom, mat));
   ...
   createPlanetGeometry: function ( planet, planetIndex ) {
      ...
      //radius *= this.planetResize
      var segments = this.planetSegments;

      var geom = new THREE.SphereBufferGeometry( radius, segments, segments );//, phiStart, phiLength, thetaStart, thetaLength );

      var position = geom.attributes.position.array;
      //var translate = planet.getPosition(100);
   ...
   getOrbitMaterial: function ( noOfPlanets ) {
      ...
      //var result = new THREE.LineBasicMaterial({ vertexColors: THREE.VertexColors,  });
      var result = new THREE.RawShaderMaterial({
         ...
         name: "OrbitShader",
         //depthTest: false,
         blending: THREE.CustomBlending, // THREE.NormalBlending,
